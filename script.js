//캔버스 객체
        ...
        function startMessage(){
            drawText(ctx, 'Enter to Start', canvas.width/2, canvas.height/2,
             "bold 30px arial", "#ffff00", "center");
            drawText(ctx, '조작 : 방향키 ←↑→↓', canvas.width/2, canvas.height/2 + 30,
             "bold 20px arial", "#ffffff", "center");
        }
        ...
        function setImage(){
            submarine = new Image();
            submarine.src = "./assets/submarine.png";
            background = new Image();
            background.src = "./assets/sea2.jpg";
        }
        ...
        function drawAll(){
            if(!STATE_START){
                return;
            }else if(STATE_GAMEOVER){ //게임 종료시 
                STATE_START = false;
                drawText(ctx, 'Game Over', canvas.width/2, canvas.height/2,
                "bold 30px arial", "#ffff00", "center");
                drawText(ctx, 'Spacebar to Restart', canvas.width/2, canvas.height/2 + 30,
                "bold 20px arial", "#ffffff", "center");
            }else{ //게임 플레이
                //배경이미지
                ...
                drawText(ctx, totalTime, canvas.width - 20, 20,
                "bold 20px arial", "#ffff00", "right");
            }
        }
